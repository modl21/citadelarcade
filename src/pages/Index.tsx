...
function LeaderboardRow({ entry, rank }: { entry: LeaderboardEntry; rank: number }) {
  ...
  return (
    <div
      className={`flex min-h-[2.75rem] items-center justify-between rounded-lg border px-3 py-1.5 transition-colors ${
        rank === 0
        ...
        <div className="min-w-0 flex-1">
          {npub ? (
            <a
              ...
              className="block truncate text-sm font-semibold tracking-tight hover:underline underline-offset-2 decoration-white/20"
            >
              {entry.lightning}
            </a>
          ) : (
            <span className="block truncate text-sm font-semibold tracking-tight">{entry.lightning}</span>
          )}
          <TopZappers lightningAddress={entry.lightning} />
        </div>
      </div>
        ...
        <header className="mb-32 space-y-12">
          <div className="space-y-6">
            ...
            <h1 className="max-w-4xl text-6xl font-black leading-[0.95] tracking-tighter text-white sm:text-7xl lg:text-8xl">
              COMPETITIVE FUN WEEKLY <span className="text-white/40 italic font-medium">GAMES</span>
            </h1>
